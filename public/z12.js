(function () {
  ...
  function addCommonBeaconProperties(beacon) {
    ...
    if (arrayUseFeature.size > 0) {
      beacon['uf'] = "".concat(Array.from(arrayUseFeature).map(function (item) {
        return "".concat(item);
      }).join(','));
    }
  }
  ...
  function checkforSriIntegrity() {
    sriIntegrity = false;
    var matchedScript = null;
    var scriptElements = document.querySelectorAll('script');
    var _iterator = _createForOfIteratorHelper(scriptElements),
      _step;
    try {
    ...
    if (matchedScript) {
      var checkIntegrityAttribute = matchedScript.getAttribute('integrity');
  ...
  function isQueryTracked(url) {
    ...
    if (defaultVars.queryTrackedDomainList.length === 0) {
      return true;
    }

    // queryTrackedDomainList contains list of url whose query parameters and fragment string should not be excluded
    // from tracking
    return matchesAny(defaultVars.queryTrackedDomainList, url);
  }
  function removeQueryAndFragmentFromUrl(url) {
    if (!url) {
      return '';
    }
    if (url === null) {
  ...
  function addCorrelationHttpHeaders(fn, ctx, traceId) {
     ...
     */
    if (defaultVars.enableW3CHeaders) {
  ...
  */
  ...
  function startResourceObservation() {
    ...
    activeResourceObserver = observeResourcePerformance({
      ...
      onEnd: function onEnd(_ref) {
        ...
        console.log("JKG:: try 1 - SPA transition took ".concat(duration.toFixed(2), "ms"));
        if (resource) {
      ...
      setTimeout(function () {
        try {
          ...
          var _performance$getEntri = performance.getEntriesByName('PageTransition'),
            ...
            measure = _performance$getEntri2[0];
          if (measure) {
            console.log("JKG:: ".concat(name, " transition duration: ").concat(measure.duration.toFixed(2), "ms"));
          }
  ...
  function _patchHistoryMethod(methodName) {
    return function (original) {
      ...
      return function patchHistoryMethod() {
        ...
        requestIdleCallback(function () {
          return endRouteAndUpdateTime("pagechange");
        });
        updateLocation(args);
        return result;
      };
  ...
  function processAutoPageDetectionCommand(input) {
    ...
    if (_typeof(guessCmd) !== 'object') return !!guessCmd;
    return {
  ...
  function processCommand(command) {
    switch (command[0]) {
      ...
      case 'queryTrackedDomainList':
        ...
        defaultVars.queryTrackedDomainList = command[1];
        break;
      ...
      case 'enableW3CHeaders':
        defaultVars.enableW3CHeaders = command[1];
        break;
      case 'reportError':
  ...
  var state$2 = {
    onEnter: function onEnter() {
      console.log('INIT1---', state$2);
      if (!fulfillsPrerequisites()) {
        console.log('INIT2---', state$2);
      ...
      try {
        console.log('INIT7---', state$2);
        addWebVitals(beacon);
      } catch (e) {
        {
          console.log('INIT8---', state$2);
          warn('Failed to capture web vitals. Will continue without web vitals', e);
        }
      }
      console.log('INIT9---', state$2);
      ...
      var traceState = checkPageLoadTraceParentAvalable();
      if (defaultVars.enableW3CHeaders && traceState) {
        console.log('INIT10---', state$2);
        defaultVars.pageLoadBackendTraceId = traceState;
      }
      console.log('INIT11---', state$2);
      ...
      console.log('before configAutoPageDetection---');
  ...
  /*
    Example:
      ...
      />
  */
  function checkPageLoadTraceParentAvalable() {
    var traceParentMeta = document.querySelector('meta[name="traceparent"]');

    // Ensure that traceparent exists and is in the correct format
    if (!traceParentMeta || !traceParentMeta.content) {
      return null;
    }
    var traceParent = traceParentMeta.content;
